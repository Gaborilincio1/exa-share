...
public class UsuarioControllerV2 {
   ...
   @GetMapping
    ...
    @GetMapping(value = "/{id}/info-completa", produces = MediaTypes.HAL_JSON_VALUE)
    public List<Object[]> getInfoCompletaPorUsuario(@PathVariable("id") Integer idUsuario) {
    return usuarioService.getInfoCompletaPorUsuario(idUsuario);
    }
    @GetMapping(value= "/resenas/{idUsuario}", produces = MediaTypes.HAL_JSON_VALUE)
    public CollectionModel<EntityModel<Map<String, Object>>> getResenasPorUsuario(@PathVariable Integer idUsuario) {
    List<Map<String, Object>> resenas = usuarioService.getResenasPorUsuario(idUsuario);
    List<EntityModel<Map<String, Object>>> models = resenas.stream()
        .map(resena -> EntityModel.of(resena,
            linkTo(methodOn(UsuarioControllerV2.class).getResenasPorUsuario(idUsuario)).withSelfRel()
        ))
        .collect(Collectors.toList());
